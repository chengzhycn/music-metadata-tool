...
- `copy_artist_to_albumartist`: 通过本次任务传入的规则，把空 albumartist 填成当前 track artist，适合榜单/单曲目录。
- `infer_artist_from_filename`: 通过本次任务传入的文件名规则，从文件名推断并填充空的 `artist` / `albumartist`；不会自动修改 `album`。
- `set_fields`: 通过本次任务传入的规则设置明确字段，例如按目录设置 `album`。
- `clear_fields`: 通过本次任务传入的规则清空明确水印字段，例如 `description=酷我音乐` 或 `publisher=PMEDIA`。
- `split_artist`: 通过本次任务传入的规则把 `artist` 拆成多值 Artist；默认不修改 `albumartist`。
...
通用字段设置、清理和多艺术家拆分规则示例：

```json
{
  "items": ["set_fields", "clear_fields", "split_artist"],
  "write": false,
  "resume": false,
  "rules": {
    "set_fields": {
      ...
      "clear": [
        {
          "fields": ["description"],
          "value_regex": "^酷我音乐$",
          "reason": "download source watermark"
        },
        {
          "fields": ["organization", "publisher", "label", "copyright", "encoder"],
          "value_regex": "^PMEDIA$",
          "reason": "release/source watermark"
        }
      ]
    },
    "split_artist": {
      "skip_patterns": ["Blue Jeans", "eason and the duo band"],
      "rules": [
        {
          "match": {
            "folder_regex": "/music/MUSIC/中文音乐/"
          },
          "separator_regex": "\\s*(?:&|/|、|，|,|\\+)\\s*",
          "reason": "split collaboration artists"
        }
      ]
    }
  }
